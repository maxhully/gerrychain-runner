...
class SlimPValueReport:
    ...
    def __init__(
        self, election, function=lambda epsilon: math.sqrt(2 * epsilon), scores=None
    ):
    ...
    def __call__(self, partition):
        ...
        if self.initial_scores is None:
            self.initial_scores = {
                name: score(election_results) for name, score in self.scores.items()
            }
    ...
    def render_without_histograms(self):
        return {
            ...
            "analysis": [self.render_score_analysis(score) for score in self.scores],
        }
    ...
    def render_score_analysis(self, score):
        ...
        return {
            "score": score,
            "interpretation": "Positive values mean an advantage for {}".format(
                party_name
            ),
...
class Histogram:
    ...
    def csv(self):
        ...
        rows = ",".join(
            (left, right, count) for (left, right), count in self.counter.items()
        )
        body = "\n".join(rows)
        return header + body
...
def get_chain_scores(chain, handlers):
    ...
    """
    return (
        {key: handler(state) for key, handler in handlers.items()} for state in chain
    )
...
def handle_scores_separately(chain, handlers):
    ...
    initialScores = {
        key: handler(chain.state) for key, handler in handlers.items() if key != "flips"
    }
...
def get_from_each(table, key):
    return [
        {header: row[header][key] for header in row if key in row[header]}
        for row in table
    ]
...
def pipe_to_table(chain, handlers, display=True, number_to_display=10, bin_interval=1):
    table = ChainOutputTable()
